...
You write Cypher; the server injects corpus isolation, caps LIMIT, and returns
...
Graph: nodes have `name` and labels (omit label if unsure). Facts live on
relationships: type(r), evidence, source_file, chunk_id, confidence in [0,1].
Do not write run_id. Never RETURN embeddings, properties(), or elementId.
Forbidden: CREATE/MERGE/SET/DELETE/REMOVE, APOC, SHOW, DDL, unbounded *,
shortestPath, any CALL except these fulltext helpers.

action=schema lists live labels and types. max_rows: default 20, max 100.
"""
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "max_rows": {
                        ...
                        "minimum": 1,
                        "maximum": 100,
                        "description": f"Output row cap, default {DEFAULT_MAX_ROWS}, max 100. Does not pre-cut aggregations.",
                    },
                },
            },
        },

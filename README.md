...
Корпус ANN и мостов S3 фильтруется по `run_id` из `server/config.yaml` (сейчас `full_corpus_20260713`; пустая строка — весь индекс и warning при старте). Эмбеддинг retrieval — локальный Ollama, задаётся `EMBED__*` в `.env`, не yaml.
...
Узлы в выдаче UNIT сводятся к четырём primary labels (`server/algorithm/models.py`):
...
После смены модели эмбеддингов пересоберите векторные индексы: `python scripts/vectorize_edges.py --run-id full_corpus_20260713 --force` и `--recreate-indexes`. Это хостовый скрипт, не способ стартовать приложение; на маленькой VM остановите контейнер `app`.
